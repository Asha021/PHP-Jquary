...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    if (empty ($err)) {
        $ae = "SELECT * FROM NewUsers WHERE Email = '$ml' AND PhoneNo = '$pn'";

        $dn = mysqli_query($conn, $ae);
            // header('location:loginSQL.php');
        
        if (mysqli_num_rows($dn) > 0) {
            $err[] = "User Already Exits";
            // header('refresh:4; u rl=logSQL.php');
        } else {
            $insert = "INSERT INTO NewUsers(UserName, PhoneNo, Email, UPassword) VALUES('$fn','$pn','$ml','$ps')";
...
<body>
    ...
    <form method="post">
        ...
        <input type="text" placeholder="User name" id="fname" name="fname" onclick="first()"><br>
        <span>
            <?php
            if ($_SERVER['REQUEST_METHOD'] == 'POST') {
                foreach ($err as $msg) {
